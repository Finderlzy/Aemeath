...
**TTS 是唯一的本地推理例外**（2026-09-13 用户确认，见 [需求](requirements.md)）。正式语音方案为本地
GPT-SoVITS：它作为本机 HTTP 服务（默认 `http://127.0.0.1:9880/tts`，api_v2 接口）运行，
Aemeath 与上游引擎都按 HTTP 客户端调用它。因此"全部使用外部模型 API"不再是 TTS 的约束：

- 引擎本身**已随上游 v1.2.1 内置**（`tts/gpt_sovits_tts.py`），不需要新写推理代码；
- 语音推理在本机执行，参考音频与音色模型文件不出本机，符合本地数据边界；
- 其余能力（对话、视觉、提取、ASR）仍按 API 方案，ASR 未随本次变更。
- 代价是引入了**本机进程依赖**：GPT-SoVITS 服务未启动时 TTS 不可用。
  这必须表现为明确的不可用状态，而不是静默退回其他引擎或静默失败。
