...
训练向导调用独立任务适配层，不能让 HTTP 请求一直等待训练结束。任务持久记录步骤、输入资源、上游版本、进度／运行状态、错误与产物；停止仅操作本应用启动的任务进程，不能停止用户原有 GPT-SoVITS 服务。训练接口与取消机制尚待验证，验证成功之前不固定使用 Gradio 调用或脚本进程。
...
| GPT-SoVITS 核查到完整工具流程，但未测可编排接口与硬件争用 | 先验证一个小训练任务的启动、进度、停止和产物，再锁定适配方式 | 训练向导生产接入阻塞于验证 |
...
### 已知既有缺陷（2026-09-14 发现，非本次改动引入）

`config/acceptance/conf.acceptance.yaml` 的 `llm_api_key` 写成 `'${AEMEATH_LLM_API_KEY}'`，
而该变量在本机取值为**纯数字** `123456`。上游 `read_yaml` 先做字符串替换、再交给 YAML
解析，替换结果被解析成 **int**，与 schema 要求的 `str` 冲突，导致服务**在校验阶段直接
无法启动**。该文件最后修改于 `bff0fd6`。修复方式是在替换后强制转为字符串，或让该变量
取值包含非数字字符；本任务未修改它，V2-T01 的验证因此改用等价的真实 HTTP 路径完成。
