...
| 生产入口回归测试 | `tests/integration/test_management_config.py`（33 项） |
...
| 合法保存并重启后实际对话使用新配置 | 通过 | 保存后经由 `load_config()` 重新读取并使用新值；保存结果通过上游 `validate_config` |
| 非法字段不损坏旧文件 | 通过 | 保存前后文件**逐字节相同**，错误指明 `base_url` |
| 并发修订冲突 | 通过 | 携带过期修订号保存返回 **409**，较新改动保留 |
| 保存失败不损坏旧文件 | 通过 | 使 `os.replace` 抛 `OSError` 后文件字节不变且仍可加载 |
| 界面区别保存成功与运行生效 | 通过 | 响应区分 `saved_revision` / `running_revision` / `restart_required`；运行修订号不因保存前进 |
...
经**真实 FastAPI app**（与生产相同的 `install_management_routes` 挂载点）在真实
HTTP 上驱动管理接口，**30/30 项检查通过**，覆盖读取、合法保存、磁盘落盘、上游 schema
复验、409 冲突、422 字段错误、密钥不外泄、凭据落盘、人设迁移、重启后取值，以及
非回环来源被 403 拒绝。

### 变异验证（9 处守卫）

改坏实现即失败的守卫：修订冲突、运行修订号、凭据脱敏、原子替换、旧人设回退、
回环限制、迁移前归档原文、归档不被覆盖、归档在读取时生效。
其中"运行修订号""凭据脱敏""旧人设回退"三处最初**并未被测试抓住**，补测后才成立。

### 回归基线

`405 passed, 7 deselected`（本次新增 33 项；此前为 372）。

### 未验证项与已知限制

- **完整服务进程未以验收配置启动成功。** `config/acceptance/conf.acceptance.yaml` 的
  回环鉴权值为纯数字 `123456`，上游 `read_yaml` 先替换 `${VAR}` 再按 YAML 解析，
  得到的是 **int**，而 schema 要求 `str`，启动在校验阶段即失败。该配置最后修改于
  `bff0fd6`，与本次改动无关，属**既有配置缺陷**；本次未修改它。
- 因此"重启后由真实服务使用新配置"是以**真实 HTTP + 真实 config loader**验证的，
  不是以整机重启验证。
- 未做真实浏览器点击操作；管理页通过 `typecheck` 与 `build:web` 构建验证。
  前端既有 586 项 vendored `WebSDK` 类型错误为**改动前基线**，本次新增代码零错误。
- 声音、记忆、Live2D 管理页不在本任务范围（V2-T02）。
