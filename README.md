...
导航共十项：概览、模型、人设、声音、声音训练、记忆、表达学习、黑话词典、Live2D、桌面与字幕。
界面截图见 [docs/images](docs/images/)，完整说明见 [启动手册 · 管理界面](docs/runbook.md#管理界面v2-t01)。
...
- **声音训练**：按六步向导（导入素材 → 清理切分 → 校对文字 → 准备训练集与训练 →
  试听 → 应用）用本机 GPT-SoVITS 训练新声音。训练在后台进行，关闭页面不会中断，
  重开管理页可继续查看状态；训练失败或停止不会替换当前可用声音，试听与应用分离。
  训练前会做环境预检，缺少权重、ffmpeg 或硬件时在对应步骤给出恢复办法。
  **素材不足时会明确拒绝**（至少 4 条、合计 10 秒以上），且**自动识别的成功不代表
  素材准确**，校对必须由你确认。详见
  [验收记录 · V2-T06](docs/acceptance.md#二十二v2-t06-完整声音训练向导2026-09-16)。
