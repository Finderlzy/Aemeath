...
客户端延迟（**产品自身指标**，来自 `logs/acceptance/turns.jsonl`，10 个有效样本）：

| 指标 | P50 | P95 | 目标 | 结论 |
| --- | --- | --- | --- | --- |
| 首段文字（提交 → 页面显示回复） | 979 ms | **1287 ms** | ≤ 5000 ms | **初步达标，样本不足，未完成验收** |
| 点击停止 → 播放器停止 | — | — | ≤ 500 ms | 未测（语音链路未验收） |
| 停止说话 → 开始播放 | — | — | ≤ 10000 ms | 未测（语音链路未验收） |

> **T00 口径校正**：上表 P50 的 **979 ms 不是统计中位数**，而是 10 个样本排序后的**第 5 个值**
> （下半中位数）。原始 JSONL 的 10 个 `client_first_text_ms` 排序为
> 505.7 / 712.3 / 814.2 / 822.6 / **979.4** / 1064.7 / 1106.3 / 1168.8 / 1275.7 / 1286.8，
> 两端中点平均为 **1022.0 ms**，算术均值为 **973.6 ms**。
> 979.4 ms 与统计中位数 1022.0 ms 相差 **42.6 ms（4.17%）**，
> 与算术均值 973.6 ms 相差 0.6%。**两种口径都不影响结论**（均远低于 5000 ms 目标）；
> 此处仅纠正口径表述，不改变任何验收判定。
> P95 = 1286.8 ms 复算确证，与表中 1287 ms 一致。

样本 10 个，**未达计划要求的 20 个**，因此状态页如实显示 `sufficient: false`。
现有 10 个样本的 P95 低于目标，只能作为初步参考；该指标**未完成验收**，
需在语音验收时补齐至 ≥20 个有效样本后再下结论。
...
- **语音链路**：麦克风采集、10 轮语音对话、5 次播放中打断、点击停止、
  播放中输入"我在上课"、课堂模式 10 轮、重启后保持课堂模式、
  关闭麦克风后停止采集——全部需要人工对麦克风说话与听感评价，未进行。
- **API ASR**：EasyCLIProxyAPI 不提供音频转写端点（404，已确认）。
  当前引擎仍是本地 SenseVoice，与嵌入的阻塞同源——供应商不提供该能力，
  不修改验收标准。
- **GPT-SoVITS TTS（本地，2026-09-13 范围调整后）**：
  TTS 不再要求云端 API，正式方案为本地 GPT-SoVITS 服务。
  本轮完成了**接入与隔离验证**：配置经上游 schema 校验通过；
  真实上游引擎对接 api_v2 替身后合成出可解码音频；
  两个上游缺陷（`streaming_mode` 默认值拼错为 `"ture"` 导致 422、
  该字段被声明为 `str` 使 YAML 布尔值校验失败）已复现并在配置层规避。
  **真实 GPT-SoVITS 服务未安装、未连通，听感未评价**——
  合成播放、打断、课堂静音与延迟四项仍待 T05 以真实设备验收。
  默认引擎**仍为 `edge_tts`**，本次未切换 `tts_model`：切换需先启动
  GPT-SoVITS api_v2 服务，属 T05 的验收动作，未获授权不提前改默认值。
  参考音频与 `prompt_text` 由环境变量注入，本次**未提供也未编造**任何
  参考音频、音色模型或转写文本，因此接入验证只用替身音频。
