...
- **两次各 30 分钟人工试用与可复现交付（2026-09-13 T07 验收更新通过）**：
  - 客户端独立补丁生成并校验通过（`docs/patches/web/0001-aemeath-web-client.patch`）；
  - 隔离环境干净复现（克隆基线、套用 4+1 个补丁、依赖补装、构建与配置检查）全部走通；
  - **第一次试用（工作陪伴 30 分钟）**：对话交互、偏好事实提取持久化、屏幕观察降级与故意不回应静默测试均表现预期；
  - **第二次试用（课堂与恢复 30 分钟）**：课堂状态零漏音、断线重连保持会话、重启后记忆追问与下课恢复语音完整流转；
  - 核心流程四步闭环实测通过；项目根目录 `README.md` 与实际入口核对一致。
